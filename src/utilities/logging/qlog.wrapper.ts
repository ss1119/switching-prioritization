...
export class QlogWrapper {
  ...
  private logToFile(evt: any[]) {
    if (this.wasClosed) {
      VerboseLogging.warn(
        "qlog was already closed, not appending!" + JSON.stringify(evt)
      );
      return;
    }
  ...
  public onPacketTX(
    packet: BasePacket,
    trigger: qlog.TransporEventTrigger = qlog.TransporEventTrigger.LINE
  ) {
    ...
    if (connIDs.src && connIDs.src != this.currentSrcConnID) {
      this.onConnectionIDUpdate(
        "SRC",
        this.currentSrcConnID,
        connIDs.src,
        "UNKNOWN"
      );
    ...
    if (spinbit && spinbit != this.currentSpinbit) {
      this.onSpinbitToggle(
        this.currentSpinbit ? this.currentSpinbit : 0,
        spinbit
      );
  ...
  protected packetToQlog(packet: BasePacket): any {
    ...
    if (
      header.getHeaderType() !== HeaderType.LongHeader &&
      header.getHeaderType() !== HeaderType.ShortHeader
    ) {
      data.message =
        "TODO: support other header types " +
        HeaderType[header.getHeaderType()];
    } else {
      data.packet_number = packet
        .getHeader()
        .getPacketNumber()!
        .getValue()
        .toDecimalString();
      ...
      if (header.getHeaderType() === HeaderType.LongHeader) {
        ...
        data.payload_length = (<LongHeader>header)
          .getPayloadLength()
          .toDecimalString();
  ...
  protected extractConnectionIDs(header: BaseHeader): {
  ...
  onFrame_MaxStreamData(
    frame: MaxStreamFrame,
    trigger: "PACKET_RX" | "PACKET_TX"
  ) {
    let evt: any = [
      123,
      qlog.EventCategory.TRANSPORT,
  ...
  public onPacketLost(
    packetNumber: Bignum,
    trigger: qlog.RecoveryEventTrigger = qlog.RecoveryEventTrigger.ACK_RX
  ) {
    let evt: any = [
      123,
  ...
  public onBytesInFlightUpdate(
    bytesInFlight: number,
    currentCWND: number,
    trigger: string = "PACKET_TX"
  ) {
    let evt: any = [
      123,
      qlog.EventCategory.RECOVERY,
  ...
  public onLossDetectionArmed(
    alarmType: string,
    lastSentHandshakeTimestamp: Date,
  ...
  public onLossDetectionTriggered(
    alarmType: string,
    metadata: Object,
    trigger: string = "TIMEOUT"
  ) {
    let evt: any = [
      123,
      qlog.EventCategory.RECOVERY,
  ...
  public onHTTPStreamStateChanged(
    streamID: Bignum,
  ...
  public onHTTPFrame_Priority(frame: Http3PriorityFrame, trigger: "TX" | "RX") {
    ...
    const PEIDString: string | undefined =
      PEID === undefined ? undefined : PEID.toString();
    const EDID: Bignum | undefined = frame.getPEID();
    const EDIDString: string | undefined =
      EDID === undefined ? undefined : EDID.toString();
  ...
  public onHTTPDependencyTreeChange(
    newTree: DependencyTree,
    trigger: "NEW" | "MOVED" | "REMOVED"
  ) {
    const evt: any = [
      123,
      "HTTP",
  ...
  public onQPACKEncoderInstruction(
    streamID: Bignum,
    instruction: Buffer,
    trigger: string
  ) {
  ...
  public onQPACKDecoderInstruction(
    streamID: Bignum,
    instruction: Buffer,
    trigger: string
  ) {
