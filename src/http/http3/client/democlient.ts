...
import {
  Http3ResourceParser,
  Http3ResourceParserEvent,
} from "./http3.resourceparser";
...
client.on(Http3ClientEvent.CLIENT_CONNECTED, () => {
  ...
  if (lookupTable === undefined) {
    ...
    resourceParser.on(
      Http3ResourceParserEvent.FILES_FOUND,
      (fileList: string[]) => {
    ...
    client.on(
      ...
      (path: string, response: Http3Message) => {
        ...
        const mimeType: string | undefined = response
          .getHeaderFrame()
      ...
      (path: string, response: Http3Message) => {
        ...
        if (!lookupTable.resources[path]) {
          VerboseLogging.error(
            "Path does not exist! " +
              JSON.stringify(lookupTable.resources, null, 4) +
              ":" +
              path
          );
          process.exit(666);
        } else {
          const relatedResources: string[] | undefined =
            lookupTable.resources[path].childrenEnd;
          if (relatedResources !== undefined) {
            for (const resource of relatedResources) {
              const metadata: Http3RequestMetadata =
                lookupTable.resources[resource];
              if (metadata.deltaStartTime !== undefined) {
                ...
                VerboseLogging.error(
                  "democlient: lookuptable resource had undefined deltaStartTime " +
                    resource
                );
                ++startedRequestCount;
                client.get(resource, authority, undefined, metadata);
        ...
        if (finishedRequestCount === startedRequestCount) {
          VerboseLogging.info(
            "All requests are fully done, ending this test run " +
              ...
              startedRequestCount +
              " === " +
              Object.keys(lookupTable.resources).length
          );
          client.DEBUGgetQlogger()!.close(); // nicely end our qlog json output
