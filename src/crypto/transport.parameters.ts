...
const TransportParameterTypeLookup: {
  [key in TransportParameterId]: TransportParameterType;
} = {
  [TransportParameterId.ORIGINAL_CONNECTION_ID]:
    TransportParameterType.ConnectionID,
  [TransportParameterId.IDLE_TIMEOUT]: TransportParameterType.uint64,
  [TransportParameterId.STATELESS_RESET_TOKEN]: TransportParameterType.Buffer,
  [TransportParameterId.MAX_PACKET_SIZE]: TransportParameterType.uint64,
  [TransportParameterId.INITIAL_MAX_DATA]: TransportParameterType.uint64,
  [TransportParameterId.INITIAL_MAX_STREAM_DATA_BIDI_LOCAL]:
    TransportParameterType.uint64,
  ...
  [TransportParameterId.INITIAL_MAX_STREAMS_BIDI]:
    TransportParameterType.uint64,
  [TransportParameterId.INITIAL_MAX_STREAMS_UNI]: TransportParameterType.uint64,
  [TransportParameterId.ACK_DELAY_EXPONENT]: TransportParameterType.uint64,
  [TransportParameterId.MAX_ACK_DELAY]: TransportParameterType.uint64,
  [TransportParameterId.DISABLE_MIGRATION]: TransportParameterType.boolean,
  [TransportParameterId.PREFERRED_ADDRESS]: TransportParameterType.Buffer,
...
export class TransportParameters {
  ...
  private tps: Map<
    ...
    ConnectionID | number | boolean | Buffer
  > = new Map<TransportParameterId, ConnectionID | number | boolean | Buffer>();
  ...
  private getTransportParametersBuffer(): Buffer {
    ...
    for (let entry of this.tps.entries()) {
      ...
      let tpType = (<any>TransportParameterTypeLookup)[
        idString
      ] as TransportParameterType;
      VerboseLogging.trace(
        "TransportParameters:getTransportParametersBuffer : adding maxsize for " +
          entry[0] +
          " // " +
          TransportParameterId[entry[0]] +
          " -> " +
          tpType
      );
      ...
      // boolean
      else maxSize += 0; // zero-length encoded, no actual value present
    }

    for (let entry of this.unknownTps.entries()) {
      VerboseLogging.trace(
        "TransportParameters:getTransportParametersBuffer : adding maxsize for greased " +
          entry[0]
      );
      maxSize += 4; // 2 bytes for type, 2 bytes for length for each entry
    ...
    VerboseLogging.trace(
      "TransportParameters:getTransportParametersBuffer : calculated MaxSize is " +
        maxSize +
        " bytes"
    );
    ...
    bufferOffset.offset = bufferOffset.buffer.writeUInt16BE(
      0xbeef,
      bufferOffset.offset
    );
    ...
    for (let tpIdString in TransportParameterId) {
      ...
      let tpIdNumber: TransportParameterId = (<any>TransportParameterId)[
        tpIdString
      ] as TransportParameterId;

      // only include explicitly set parameters
      if (!this.tps.has(tpIdNumber)) {
        // will only return true if it was set before + if tptype is one of the string keys returning an int (since this.tps indexes on ints)
        continue;
      }

      let tpType: TransportParameterType = (<any>TransportParameterTypeLookup)[
        tpIdString
      ] as TransportParameterType;
      VerboseLogging.trace(
        "getTransportParametersBuffer: encoding " +
          TransportParameterId[tpIdNumber] +
          " with internal type " +
          TransportParameterType[tpType]
      );
      ...
      switch (tpType) {
        case TransportParameterType.uint64:
          ...
          bufferOffset = this.writeIdAndLength(
            tpIdNumber,
            bufferOffset.buffer,
            bufferOffset.offset,
            encodedBuffer.byteLength
          );
          bufferOffset.offset += encodedBuffer.copy(
            bufferOffset.buffer,
            bufferOffset.offset
          );
          break;
        ...
        case TransportParameterType.Buffer:
          ...
          bufferOffset.offset += bufval.copy(
            bufferOffset.buffer,
            bufferOffset.offset
          );
          break;
        ...
        case TransportParameterType.ConnectionID:
          ...
          bufferOffset.offset += cidval.copy(
            bufferOffset.buffer,
            bufferOffset.offset
          );
          break;
    ...
    for (let entry of this.unknownTps) {
      VerboseLogging.trace(
        "getTransportParametersBuffer: encoding greased TP : " + entry[0]
      );
      ...
      bufferOffset.offset += entry[1].copy(
        bufferOffset.buffer,
        bufferOffset.offset
      );
    ...
    VerboseLogging.trace(
      "getTransportParametersBuffer: finalOutput is " +
  ...
  private writeIdAndLength(
    id: number,
    targetBuffer: Buffer,
    targetOffset: number,
    tpValueByteLength: number
  ): BufferOffset {
   ...
   */
  public static fromExtensionBuffer(
    isServer: boolean,
    buffer: Buffer
  ): TransportParameters {
    VerboseLogging.trace(
      "TransportParameters:fromExtensionBuffer : " + buffer.toString("hex")
    );

    //let values: { [index: number]: any; } = [];
    let offset = 0;
    let transportParameters =
      TransportParameters.getEmptyTransportParameters(isServer);
    ...
    if (totalLength != buffer.byteLength - 2) {
      VerboseLogging.warn(
        "TransportParameters:fromExtensionBuffer : buffer length != length field in the TPs! " +
          buffer.byteLength +
          " != " +
          totalLength
      );
    ...
    while (offset < buffer.byteLength) {
      ...
      if (validTp) {
        let tpIdString: TransportParameterId = (<any>TransportParameterId)[
          tpIdNumber
        ] as TransportParameterId;
        let tpType: TransportParameterType = (<any>(
          TransportParameterTypeLookup
        ))[tpIdString] as TransportParameterType;
        VerboseLogging.trace(
          "fromExtensionBuffer: decoding " +
            TransportParameterId[tpIdNumber] +
            " with internal type " +
            TransportParameterType[tpType]
        );
        ...
        if (transportParameters.tps.has(tpIdNumber)) {
          VerboseLogging.error(
            "fromExtensionBuffer: decoding : Duplicate TP detected " +
              TransportParameterId[tpIdNumber] +
              ". This MUST result in connection closure, but we don't do that yet!"
          );
        }
        ...
        switch (tpType) {
          case TransportParameterType.uint64:
            ...
            if (decodedVarint.offset - offset != valueLength)
              VerboseLogging.warn(
                "fromExtensionBuffer : VLIE length was not the same as prepended length! " +
                  (decodedVarint.offset - offset) +
                  " != " +
                  valueLength
              );
            ...
            VerboseLogging.trace(
              "fromExtensionBuffer: adding uint64 " +
                TransportParameterId[tpIdNumber] +
                " = " +
                tpValue.toDecimalString()
            );
            ...
            try {
              ...
              VerboseLogging.error(
                "TransporParameters:fromExtensionBuffer : uint64 was actually larger than 2^53, truncating to MAX_SAFE_INTEGER! " +
                  tpValue.toDecimalString() +
                  ", " +
                  tpValue.toString("hex")
              );
          ...
          case TransportParameterType.boolean:
            VerboseLogging.trace(
              "fromExtensionBuffer: adding boolean " +
                TransportParameterId[tpIdNumber] +
                " = " +
                true
            );
          ...
          case TransportParameterType.Buffer:
            ...
            VerboseLogging.trace(
              "fromExtensionBuffer: adding buffer " +
                TransportParameterId[tpIdNumber] +
                " = " +
                tpBufValue.toString("hex")
            );
          ...
          case TransportParameterType.ConnectionID:
            ...
            VerboseLogging.trace(
              "fromExtensionBuffer: adding ConnectionID " +
                TransportParameterId[tpIdNumber] +
                " = " +
                tpCidValue.toString("hex")
            );
      ...
      // invalid tp id: unknown tp, save it in separate Map
        ...
        VerboseLogging.trace(
          "fromExtensionBuffer: Unknown TP : " +
            tpIdNumber +
            " = " +
            tpValue.toString("hex")
        );
  ...
  public static getEmptyTransportParameters(
    isServer: boolean
  ): TransportParameters {
    return new TransportParameters(isServer);
  }

  public static getDefaultTransportParameters(
    isServer: boolean
  ): TransportParameters {
    ...
    transportParameters.setTransportParameter(
      TransportParameterId.INITIAL_MAX_STREAM_DATA_BIDI_LOCAL,
    ...
    transportParameters.setTransportParameter(
      ...
      Constants.DEFAULT_ACK_DELAY_EXPONENT
    );
    transportParameters.setTransportParameter(
      TransportParameterId.MAX_ACK_DELAY,
      Constants.DEFAULT_MAX_ACK_DELAY
    );
    transportParameters.setTransportParameter(
      TransportParameterId.ACTIVE_CONNECTION_ID_LIMIT,
      Constants.DEFAULT_ACTIVE_CONNECTION_ID_LIMIT
    );

    if (Constants.DEFAULT_DISABLE_MIGRATION)
      transportParameters.setTransportParameter(
        TransportParameterId.DISABLE_MIGRATION,
        Constants.DEFAULT_DISABLE_MIGRATION
      );

    if (isServer) {
      transportParameters.setTransportParameter(
        TransportParameterId.INITIAL_MAX_STREAMS_BIDI,
        Constants.DEFAULT_MAX_STREAM_CLIENT_BIDI
      );
      transportParameters.setTransportParameter(
        TransportParameterId.INITIAL_MAX_STREAMS_UNI,
        Constants.DEFAULT_MAX_STREAM_CLIENT_UNI
      );
      // TODO: better to calculate this value
      transportParameters.setTransportParameter(
        TransportParameterId.STATELESS_RESET_TOKEN,
        Bignum.random("ffffffffffffffffffffffffffffffff", 16).toBuffer()
      );
    } else {
      transportParameters.setTransportParameter(
        TransportParameterId.INITIAL_MAX_STREAMS_BIDI,
        Constants.DEFAULT_MAX_STREAM_SERVER_BIDI
      );
      transportParameters.setTransportParameter(
        TransportParameterId.INITIAL_MAX_STREAMS_UNI,
        Constants.DEFAULT_MAX_STREAM_SERVER_UNI
      );
    }

    if (Constants.DEBUG_greaseTransportParameters) {
      transportParameters.unknownTps.set(
        0xffff,
        Buffer.from([0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xff])
      );
      transportParameters.unknownTps.set(
        0x1234,
  ...
  public toJSONstring(prettyPrint: boolean = false) {
    ...
    for (let entry of this.tps.entries()) {
      ...
      if (tpType === TransportParameterType.uint64)
        ...
        known[idString] =
          "" + (entry[1] as ConnectionID).toBuffer().toString("hex");
      // boolean
      else known[idString] = "" + (entry[1] as boolean);
    }
