...
import {
  ...
  Version,
  ConnectionID,
} from "../packet/header/header.properties";
...
export class Client extends Endpoint {
  ...
  private init(onSocketReady: () => void): void {
    let onSocketBound = () => {
      ...
      socket.on(QuickerEvent.ERROR, (err) => {
        this.handleError(this.connection, err);
      });
      socket.on(QuickerEvent.NEW_MESSAGE, (msg, rinfo) => {
        this.onMessage(msg);
      });
  ...
  private setupConnectionEvents() {
    ...
    this.connection.on(
      ConnectionEvent.CREATED_STREAM,
    ...
    this.connection.on(
      ...
      (packet: BufferedPacket) => {
        ...
        VerboseLogging.debug(
          ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>////////////////////////////// Client: PROCESSING BUFFERED PACKET //////////////////////////////// "
        );
        VerboseLogging.info(
          "client:BufferedPacketReadyForDecryption: raw message from the wire : " +
  ...
  public request(
    request: Buffer,
    streamType: StreamType.ClientBidi | StreamType.ClientUni
  ): QuicStream {
    var stream: Stream = this.connection
      .getStreamManager()
      .getNextStream(streamType);
    if (this.connected) {
  ...
  public createStream(
    ...
    if (this.connected) {
      return new QuicStream(
        this.connection,
        this.connection.getStreamManager().getNextStream(streamType)
      );
    } else {
  ...
  public close(closePhrase: string | undefined = undefined) {
    ...
    this.handleError(
      this.connection,
      new QuicError(
        ConnectionErrorCodes.NO_ERROR,
        closePhrase !== undefined
          ? closePhrase
  ...
  private onMessage(msg: Buffer): any {
    ...
    VerboseLogging.trace(
      "client:onMessage: message length in bytes: " + msg.byteLength
    );
    VerboseLogging.info(
      "client:onMessage: raw message from the wire : " + msg.toString("hex")
    );
    ...
    try {
      ...
      VerboseLogging.error(
        "Client:onMessage: could not parse headers! Ignoring packet. " +
          err.toString() +
          " -> " +
          msg.toString("hex")
      );
    ...
    VerboseLogging.debug(
      "Client:onMessage: Message contains " +
        packets.length +
  ...
  private processPackets(packets: PartiallyParsedPacket[], receivedTime: Time) {
    try {
      ...
      packets.forEach((packet: PartiallyParsedPacket) => {
        ...
        if (decryptedHeaderPacket !== undefined) {
          ...
          setImmediate(() => {
            ...
            VerboseLogging.debug(
              "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<////////////////////////////// Client: done handling packet //////////////////////////////// "
            );
          });
        } else
          VerboseLogging.info(
            "Client:processPackets: could not decrypt packet, buffering till later"
          );
      });
      ...
      if (
        ...
        VerboseLogging.info(
          "Client:processPackets : caught IGNORE_PACKET_ERROR : " + err
        );
        return;
      }
      if (
    ...
    VerboseLogging.debug(
      "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<////////////////////////////// Client: done processing these packets //////////////////////////////// "
    );
  }
}
