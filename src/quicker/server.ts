...
export class Server extends Endpoint {
  ...
  public static createServer(options?: any) {
    ...
    if (options.secureContext === undefined) {
      options.secureContext = server.createServerSecureContext(
        options.key,
        options.cert
      );
    }
  ...
  public createStream(
    ...
    streamType: StreamType.ServerBidi | StreamType.ServerUni
  ): QuicStream {
    // TODO Check connection ownership
    return new QuicStream(
      connection,
      connection.getStreamManager().getNextStream(streamType)
    );
  }
  ...
  private init(socketType: SocketType) {
    ...
    VerboseLogging.info(
      "Server:init: Creating a socket of type " +
        socketType +
        " @ " +
        this.hostname
    );
    server.on(QuickerEvent.NEW_MESSAGE, (msg, rinfo) => {
      this.onMessage(msg, rinfo);
    });
    server.on(QuickerEvent.CONNECTION_CLOSE, () => {
      this.handleClose();
    });
  ...
  private setupConnectionEvents(connection: Connection) {
    ...
    connection.on(ConnectionEvent.DRAINING, () => {
      this.emit(
        QuickerEvent.CONNECTION_DRAINING,
        connection.getSrcConnectionID().toString()
      );
    });
    connection.on(ConnectionEvent.CLOSE, () => {
      this.connectionManager.deleteConnection(connection);
      this.emit(
        QuickerEvent.CONNECTION_CLOSE,
        connection.getSrcConnectionID().toString()
      );
    });
    connection.on(
      ConnectionEvent.BufferedPacketReadyForDecryption,
      (packet: BufferedPacket) => {
        ...
        VerboseLogging.debug(
          ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>////////////////////////////// Server: PROCESSING BUFFERED PACKET //////////////////////////////// "
        );
        VerboseLogging.info(
          "server:BufferedPacketReadyForDecryption: raw message from the wire : " +
            packet.packet.fullContents.toString("hex")
        );
        this.processPackets(
          [packet.packet],
          undefined,
  ...
  private onMessage(msg: Buffer, rinfo: RemoteInfo | undefined): any {
    ...
    VerboseLogging.trace(
      "server:onMessage: message length in bytes: " + msg.byteLength
    );
    VerboseLogging.info(
      "server:onMessage: raw message from the wire : " + msg.toString("hex")
    );
    ...
    try {
      ...
      VerboseLogging.error(
        "Server:onMessage: could not parse headers! Ignoring packet. " +
          err.toString() +
          " // " +
    ...
    VerboseLogging.debug(
      "Server:onMessage: Message contains " +
        packets.length +
        " independent packets (we think)"
    );
  ...
  private processPackets(
    packets: PartiallyParsedPacket[],
    rinfo: RemoteInfo | undefined,
    receivingConnection: Connection | undefined,
    receivedTime: Time
  ) {
    packets.forEach((packet: PartiallyParsedPacket) => {
      let connection: Connection | undefined = undefined;
      try {
        if (receivingConnection) connection = receivingConnection;
        else
        ...
        let decryptedHeaderPacket: PartiallyParsedPacket | undefined =
          this.headerHandler.decryptHeader(
            connection,
            packet,
            EndpointType.Client,
            receivedTime
        ...
        if (decryptedHeaderPacket !== undefined) {
          let handledHeader: PartiallyParsedPacket | undefined =
            this.headerHandler.handle(
              connection,
              decryptedHeaderPacket,
              EndpointType.Client
            );
          let fullyDecryptedPacket: BasePacket = this.packetParser.parse(
            connection,
            handledHeader!,
            EndpointType.Client
          );
          ...
          setImmediate(() => {
            VerboseLogging.debug(
              ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>////////////////////////////// Server: handling packet  //////////////////////////////// "
            );
            this.packetHandler.handle(
              connection!,
              fullyDecryptedPacket,
              receivedTime
            );
            VerboseLogging.debug(
              "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<////////////////////////////// Server: done handling packet //////////////////////////////// "
            );
          });
        } else
          VerboseLogging.info(
            "Server:processPackets: could not decrypt packet, buffering till later"
          );
        ...
        if (connection === undefined) {
          ...
          VerboseLogging.error(
            "Server:processPackets : message received but ignored because we only expect an INITIAL packet at this point. TODO: buffer this until the initial is received, then re-process"
          );
          return;
        } else if (
          err instanceof QuicError &&
          err.getErrorCode() === ConnectionErrorCodes.VERSION_NEGOTIATION_ERROR
        ) {
          VerboseLogging.debug(
            "Server:processPackets : VERSION_NEGOTIATION_ERROR : unsupported version in INITIAL packet : " +
              err +
              " : re-negotiating"
          );
          ...
          connection
            .getEncryptionContext(EncryptionLevel.INITIAL)!
            .getPacketNumberSpace()
            .setHighestReceivedNumber(new PacketNumber(new Bignum(0)));

          let versionNegotiationPacket =
            PacketFactory.createVersionNegotiationPacket(connection);
          connection.sendPacket(versionNegotiationPacket);
          return;
        } else if (
          err instanceof QuickerError &&
          err.getErrorCode() === QuickerErrorCodes.IGNORE_PACKET_ERROR
        ) {
          VerboseLogging.info(
            "Server:processPackets : caught IGNORE_PACKET_ERROR : " + err
          );
          return;
        } else {
    ...
    VerboseLogging.debug(
      "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<////////////////////////////// Server: done processing these packets //////////////////////////////// "
    );
  }
  ...
  private createConnectionManager() {
    ...
    this.connectionManager.on(
      ConnectionManagerEvents.CONNECTION_CREATED,
      (connection: Connection) => {
